...
export default function SearchPanel({ onClose }: SearchPanelProps): React.ReactElement {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                const res = await axiosInstance.get(`/posts/channel/${selectedChannelId}`);
                console.log(`게시물 API 응답 (/posts/channel/${selectedChannelId}):`, res.data);
                setPosts(res.data);
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const filtered = users.filter((user) => user.fullName.toLowerCase().includes(searchInput.toLowerCase()));
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            {filteredUsers.length > 0 ? (
                                ...
                                <p className="text-sm text-gray-500 text-center">일치하는 사용자가 없습니다.</p>
                            )}
                        </>
                    ) : (
                        <>
                            ...
                            {Array.isArray(filteredPosts) && filteredPosts.length > 0 ? (
                                ...
                                <p className="text-sm text-gray-500 text-center">일치하는 게시글이 없습니다.</p>
                            )}
                        </>
                    )}
                </div>
                    ...
                    <div className="mt-6">
                        ...
                        <ul>
                            {recentPostSearches.map((post) => (
                                ...
                                >
                                    <div className="flex items-center gap-2 cursor-pointer hover:text-blue-600">
                                        <span className="text-gray-800">{post.title || '제목 없음'}</span>
                                    </div>
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
