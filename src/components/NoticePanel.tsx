...
function NoticePanel({ onClose, isOpen }: NoticePanelProps): React.ReactElement {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || err.message || '알림을 불러올 수 없습니다.');
            setNotifications([]);
        } finally {
            setLoading(false);
        }
    ...
    const getNotificationMessage = (notice: Notification): string => {
        ...
        if (notice.notificationType) {
            switch (notice.notificationType) {
                case 'LIKE':
                    message = `${actor?.fullName || '누군가'}님이 회원님의 게시글을 좋아합니다.`;
                    break;
                case 'COMMENT':
                    const commentContent = notice.comment?.content;
                    if (commentContent) {
                        message = `${actor?.fullName || '누군가'}님이 댓글을 남겼습니다: "${commentContent.substring(
                            0,
                            20
                        )}${commentContent.length > 20 ? '...' : ''}"`;
                    } else {
                        message = `${actor?.fullName || '누군가'}님이 회원님의 게시글에 댓글을 남겼습니다.`;
                    }
                    break;
                case 'FOLLOW':
                    message = `${actor?.fullName || '누군가'}님이 회원님을 팔로우하기 시작했습니다.`;
                    break;
            }
        } else if (notice.like) {
            message = `${actor?.fullName || '누군가'}님이 회원님의 게시글을 좋아합니다.`;
            ...
            const commentContent = notice.comment?.content;
            if (commentContent) {
                message = `${actor?.fullName || '누군가'}님이 댓글을 남겼습니다: "${commentContent.substring(0, 20)}${
                    commentContent.length > 20 ? '...' : ''
                }"`;
            } else {
                message = `${actor?.fullName || '누군가'}님이 회원님의 게시글에 댓글을 남겼습니다.`;
    ...
    return (
        <div
            className="px-4 flex flex-col text-sm bg-[var(--color-main-bg)] shadow-md h-full w-[360px]"
            style={{ boxShadow: '4px 0 6px rgba(0, 0, 0, 0.15)' }}
        >
            <div className="flex justify-between items-center mb-2 pt-4">
                <h2 className="text-xl font-bold text-[var(--color-notice-text)]">알림</h2>
                    ...
                    {notifications.map((notice) => (
                        ...
                        >
                            ...
                            <div className="flex-grow text-xs">
                                <p className={`text-gray-800 leading-relaxed ${!notice.seen ? 'font-semibold' : ''}`}>
