...
export default function PostModal({ post, onClose, user, onSaved, isMyProfile }: PostModalProps) {
    if (!post) return null;
    ...
    const [locationsInput, setLocationsInput] = useState(editableTitle.locations?.join(', ') || '');
    const [hotelsInput, setHotelsInput] = useState(editableTitle.hotels?.join(', ') || '');
    const [tagsInput, setTagsInput] = useState(editableTitle.tags?.join(', ') || '');
    ...
    const handleCloudinaryUpload = async (files: File[]) => {
        ...
        const successful = uploaded.filter((item): item is UploadedImage => item !== null);
    ...
    useEffect(() => {
        if (post?.title) {
            try {
                ...
                setUploadedImages((parsed.uploadedImages ?? []).map((url) => ({ url, publicId: '' })));
            } catch (e) {
                        ...
                        <div className="w-[50vw]">
                            <div className="flex justify-between items-center">
                                {isEditing ? (
                                    <div className="flex items-center gap-3 w-full">
                                        <span className="w-[40px] text-center text-[17px]">제목</span>
                                        <input
                            ...
                            <div className="flex gap-3 mb-[10px] w-[100%] py-[5px]">
                                {isEditing ? (
                                    <div className="flex items-center gap-2 w-full ">
                                        ...
                                        <span className="w-[40px] text-center text-[14px]">위치</span>
                                        <input
                                ...
                                {isEditing ? (
                                    <div className="flex items-center gap-2">
                                        <span className="w-[40px] text-center text-[14px]">비용</span>
                                        <input
                                    ...
                                    <div className="ml-auto text-[12px]">총 비용: {parsedTitle.cost}</div>
                                )}
                            </div>
                            ...
                            <div className="flex gap-3 text-[15px]">
                                {isEditing ? (
                                    <div className="flex items-center gap-2 w-full">
                                        ...
                                        <span className="w-[40px] text-center text-[14px]">숙소</span>
                                        <input
                                            className="w-full min-h-[50px] border rounded p-2 text-[15px]"
                                            value={hotelsInput}
                            ...
                            <div className="min-h-[130px] my-4 overflow-auto border-t-2 border-[var(--color-lightGray)] py-3">
                                {isEditing ? (
                                    ...
                                    <p className="text-[15px] whitespace-pre-line">{parsedTitle.context}</p>
                                )}
                            </div>
                        </div>
                    </div>
